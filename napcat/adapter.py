"""NapCat OneBot 11 adapter for Hermes.

Install this file as ``gateway/platforms/napcat_adapter.py`` in an existing
Hermes Agent source checkout.

Configuration is documented in the repository README.
"""
    ...
    async def _process_message(self, event: dict) -> None:
        ...
        if is_group:
            ...
            if self._group_policy == "allowlist":
                # group_allow_from contains group IDs; allow_from is the
                # optional sender allowlist fallback for compatibility.
                if self._group_allow_from and group_id not in self._group_allow_from:
                    return
                if not self._group_allow_from and self._allow_from and sender_id not in self._allow_from:
                    return
        else:
