                ...
                if adapter is not None:
                    ...
                    if adapter_group_allowed:
                        ...
                        if "*" in allowed or source.chat_id in allowed:
                            return True
                    # NapCat keeps its explicit group allowlist in
                    # _group_allow_from. Treat a message that passed that
                    # adapter-owned group gate as authorized here too.
                    napcat_group_allowed = getattr(adapter, "_group_allow_from", None)
                    if napcat_group_allowed:
                        allowed = _coerce_allow_set(napcat_group_allowed)
